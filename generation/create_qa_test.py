...
def choose_one_answer(qa_pair: dict, accuracy: float, include_context: bool, include_samples: bool, dataset: str) -> dict | None:
    ...
    if dataset == 'medquad':
        if 'context' in qa_pair.keys() and include_context:
            qa_test['context'] = qa_pair['context']
...
def generate_test(qa_data: list[dict], accuracy: float, include_context: bool, include_samples: bool, dataset: str) -> list[dict]:
    test_data = []
    for row in qa_data:
        if new_row := (choose_one_answer(row, accuracy, include_context, include_samples, dataset)) and new_row['answer'] != "":
...
def main() -> None:
    ...
    parser.add_argument('--accuracy', type=float, default=0.5, help='Proportion of right answers to be chosen (0 to 1)')
    ...
    with open(outfile, 'w') as f:
        json.dump(generate_test(qa_data, args.accuracy, args.context, args.samples, dataset), f, indent=4)
